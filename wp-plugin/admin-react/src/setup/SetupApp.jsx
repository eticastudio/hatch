...
function PageHeading({ title, lede }) {
	return (
		<div>
			...
			{lede && (
				<p style={{ fontSize: 14, color: 'var(--hx-subtle)', lineHeight: 1.6, marginTop: 8, marginBottom: 0 }}>
					{lede}
				</p>
			)}
		</div>
		...
		<div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
			...
			<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
				<a
					...
					style={{ fontSize: 13, color: 'var(--hx-subtle)', textDecoration: 'none' }}
				>
...
function CheckRow({ check: c, last }) {
	...
	return (
		...
		>
			...
			<div style={{ flex: 1 }}>
				<div style={{ fontSize: 13, fontWeight: 500, color: state === 'fail' ? fg : 'var(--hx-fg)', lineHeight: 1.4 }}>
					{c.label || c.l}
				</div>
				{c.note && (
					<div style={{ fontSize: 12, color: 'var(--hx-subtle)', marginTop: 3, lineHeight: 1.5 }}>
						{c.note}
					</div>
				)}
			</div>
...
function Step2Theme({ boot, onBack }) {
	const themes = [
		{ id: 'blog',       col: '#3b82f6', name: 'Blog',       desc: 'Reading-first, minimal. Personal blogs, news, magazines.',         preview: 'Blog' },
		{ id: 'tech',       col: '#8b5cf6', name: 'Tech',       desc: 'Code blocks plus dark mode. Next.js / developer blog vibe.',       preview: 'Tech' },
		{ id: 'data',       col: '#0d9488', name: 'Data',       desc: 'Sidebar nav and search. Versioned docs and knowledge bases.',     preview: 'Data' },
		{ id: 'astropaper', col: '#ff6b00', name: 'AstroPaper', desc: 'Minimal clean blogging theme. Lightweight, featured images.',     preview: 'AstroPaper' },
		{ id: 'astrowind',  col: '#2563eb', name: 'AstroWind',  desc: 'Business / marketing theme with hero, features, CTA sections.',   preview: 'AstroWind' },
		{ id: 'astro-nano', col: '#737373', name: 'Astro Nano', desc: 'Ultra minimal. Just words. No distractions, no sidebars.',         preview: 'Astro Nano' },
	];
				...
				<HxCard status="info" style={{ padding: '14px 16px' }}>
					<div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
						...
						<div style={{ flex: 1, fontSize: 13, color: 'var(--hx-fg)', lineHeight: 1.55 }}>
							<strong style={{ fontWeight: 600 }}>Going headless</strong>{' '}
...
function BrokerForm({ provider, tokenName, tokenUrl, tokenUrlLabel, tokenPagePrompt, adminPostUrl, deployNonce }) {
	...
	return (
		<div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
			...
			<p style={{ fontSize: 13, color: 'var(--hx-muted)', lineHeight: 1.55, margin: '4px 0 10px' }}>
				{tokenPagePrompt}
			</p>
			<div>
				<HxBtn href={tokenUrl}>
			...
			>
				...
				<label className="hx-checkbox" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--hx-muted)', cursor: 'pointer' }}>
					<input
						type="checkbox"
						name="save_token"
						value="1"
				...
				<p style={{ fontSize: 11, color: 'var(--hx-subtle)', lineHeight: 1.5, margin: '4px 0 0' }}>
...
function Step3Deploy({ boot, onBack }) {
	...
	return (
		<div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
					...
					return (
						<HxCard key={o.id} style={{ padding: 0, overflow: 'hidden' }}>
							...
							>
								...
								<div style={{ flex: 1 }}>
									<div style={{ fontSize: 14, fontWeight: 600, color: 'var(--hx-fg)', display: 'flex', alignItems: 'center', gap: 8 }}>
										{o.label}
										...
										<>
											<div style={{ marginBottom: 16, maxWidth: 360 }}>
												<HxSeg
													...
													options={[
														{ value: 'agent', label: 'Install agent' },
														{ value: 'url',   label: 'Paste existing URL' },
													]}
												/>
											</div>
											...
											{selfMode === 'agent' && (
												<>
													<p style={{ fontSize: 13, color: 'var(--hx-muted)', lineHeight: 1.6, margin: '0 0 10px' }}>
													...
													{!setup.appPassword && (
											<HxCard status="warning" style={{ padding: '10px 12px', marginBottom: 12, fontSize: 12, color: 'var(--hx-fg)', lineHeight: 1.5 }}>
													...
													<p style={{ fontSize: 12, color: 'var(--hx-subtle)', lineHeight: 1.55, margin: '10px 0 0' }}>
													...
													<details style={{ marginTop: 14 }}>
														<summary style={{ cursor: 'pointer', listStyle: 'none', fontSize: 12, color: 'var(--hx-subtle)', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
											...
											{selfMode === 'url' && (
												<form method="post" action={boot.adminPostUrl}>
													<input type="hidden" name="action" value="hatch_save_manual_target" />
													<input type="hidden" name="_wpnonce" value={setup.nonces?.save_manual_target || ''} />
													<p style={{ fontSize: 13, color: 'var(--hx-muted)', lineHeight: 1.6, margin: '0 0 12px' }}>
														Already running an Astro site? Paste the URL and Hatch wires WordPress to it.
													</p>
													<div style={{ fontSize: 12, fontWeight: 600, color: 'var(--hx-muted)', marginBottom: 6 }}>Frontend URL</div>
													<HxInp
														name="hatch_frontend_url"
														placeholder="https://your-site.com or http://localhost:4321"
														value={manualUrl}
			...
			<p style={{ textAlign: 'center', marginTop: 6, color: 'var(--hx-subtle)', fontSize: 12 }}>
	...
	return (
		<div className="hatch-react" style={{ minHeight: '100vh', paddingBottom: 60, background: 'var(--hx-bg)' }}>
			...
			<div style={{ textAlign: 'center', padding: '40px 24px 0' }}>
				...
				<p style={{ fontSize: 13, color: 'var(--hx-subtle)', marginTop: 6 }}>
